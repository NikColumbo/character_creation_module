...
def choice_char_class() -> str:
    ...
    while approve_choice != 'y':
        char_class = input('Введи название класса, за которого хочешь играть:'
                           'Воитель — warrior, Маг — mage, Лекарь — healer:')
        if char_class == 'warrior':
            print('Воитель — воин ближнего боя. Сильный и выносливый.')
        if char_class == 'mage':
            print('Маг —  воин дальнего боя. Обладает высоким интеллектом.')
        if char_class == 'healer':
            print('Лекарь —  заклинатель. Черпает силы из природы и веры.')
        confirm = ('Нажми (Y), чтобы подтвердить выбор ')
        change = (', или любую другую кнопку, чтобы выбрать другой класс')
    approve_choice = input(f'{confirm}{change}').lower()
    return char_class
